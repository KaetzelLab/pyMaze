...
class Pycboard(Pyboard):
    ...
    def setup_state_machine(self, sm_name, sm_dir=tasks_dir, uploaded=False):
        ...
        self.sm_info = {'name'  : sm_name,
                        ...
                        'coordinates':self.get_coordinates()} # {name: repr(value)}
        if self.data_logger:
    ...
    def get_coordinates(self):
        '''Return variables as a dictionary {variable_name: value}'''
        return eval(self.exec('fw.get_coordinates()').decode().strip())
    ...
    def process_data(self):
        ...
        while self.serial.inWaiting() > 0:
            ...
            if new_byte == b'A': # Analog data, 13 byte header + variable size content.
            ...
            elif new_byte in (b'P', b'V', b'C'): # User print statement or set variable, 8 byte data header + variable size content.
                data_header = self.serial.read(8)
                ...
                if new_byte == b'c': # Store new variable value in sm_info
                    v_str = data_bytes.decode().split(' ', 1)
                    print(eval(v_str))
    ...
    def set_coordinates(self, c_name, c_value):
        assert c_name in self.sm_info['coordinates'], 'Invalid Co-ordinates name'
        c_str = repr(c_value)
        if self.framework_running:  # Set variable with serial command.
            data = repr((c_name, c_str)).encode() + b'c'
            data_len = len(data).to_bytes(2, 'little')
            checksum = sum(data).to_bytes(2, 'little')
            self.serial.write(b'c' + data_len + data + checksum)
            return None
        else:  # Set variable using REPL.
            checksum = sum(c_str.encode())
            return eval(self.eval("state_machine._set_variable({}, {}, {})"
                                  .format(repr(c_name), repr(c_str), checksum)).decode())
