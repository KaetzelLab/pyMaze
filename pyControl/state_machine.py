...
class State_machine():
    ...
    def _process_event(self, event):
        ...
        if self.event_dispatch_dict[
            self.current_state]:  # If state machine has event handler function for current state.
    ...
    def _set_coordinates(self, c_name, c_str, checksum=None):
        if checksum:
            str_sum = sum(c_str) if type(c_str) is bytes else sum(c_str.encode())
            if not str_sum == checksum:
                return False  # Bad checksum.
        try:
            setattr(self.smd.c, c_name, eval(c_str))
            return True  # Variable set OK.
        except Exception:
            return False  # Bad variable name or invalid value string.
