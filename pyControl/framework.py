...
def get_coordinates():
    # Print first instantiated state machines variables as dict {v_name: repr(v_value)}
    print({k: repr(v) for k, v in state_machine.smd.c.__dict__.items()})
...
def recieve_data():
    ...
    if new_byte == b'\x03':  # Serial command to stop run.
        ...
        if not checksum == (sum(data) & 0xFFFF):
            return  # Bad checksum.
        if data[-1:] == b'c':  # Set variable.
            c_name, c_str = eval(data[:-1])
            state_machine._set_coordinates(c_name, c_str)
